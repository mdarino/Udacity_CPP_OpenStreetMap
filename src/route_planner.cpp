...
RoutePlanner::RoutePlanner(RouteModel &model, float start_x, float start_y, float end_x, float end_y): m_Model(model) {
    ...
    // Store the nodes you find in the RoutePlanner's start_node and end_node attributes.  DONE!!!
...
void RoutePlanner::AddNeighbors(RouteModel::Node *current_node) {
    ...
    for (int i = 0; i < current_node->neighbors.size(); i++) {
        float g_distance = current_node->g_value + current_node->distance(*(current_node->neighbors[i]));
        current_node->neighbors[i]->g_value = g_distance;
        ...
        RoutePlanner::open_list.emplace_back(current_node->neighbors[i]);
    }
}
...
RouteModel::Node *RoutePlanner::NextNode() {
    RouteModel::Node *node_lower = nullptr;
    if (RoutePlanner::open_list.size() > 0){
      ...
      node_lower = RoutePlanner::open_list.back();
